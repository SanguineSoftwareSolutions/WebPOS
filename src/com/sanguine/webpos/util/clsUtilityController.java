...
import com.sanguine.webpos.bean.clsBillItemTaxDtl;
import com.sanguine.webpos.bean.clsBuyPromotionItemDtl;
import com.sanguine.webpos.bean.clsGetPromotionItemDtl;
...
//                + "order by a.strTaxOnTax,a.strTaxDesc");
                ...
                if (funCheckOperationType(opType, operationTypeForTax))
                {
                    if (funFindSettlementForTax(taxCode, settlementCode))
                    {
                        ...
                        if (taxOnGD.equals("Gross"))
                        {
                            ...
                            {
                                clsItemDtlForTax objItemDtl = arrListItemDtl.get(j);
                ...
                if (funCheckAreaCode(taxAreaCodes, billAreaCode))
                {
                    if (funCheckOperationType(opType, operationTypeForTax))
                    {
                        if (funFindSettlementForTax(spTaxOnTaxCode[t], settlementCode))
                        {
                            ...
                            sqlTaxOnTax.append("select a.strTaxCode,a.strTaxDesc,a.strTaxOnSP,a.strTaxType,a.dblPercent,a.dblAmount,a.dteValidFrom,a.dteValidTo,a.strTaxOnGD,a.strTaxCalculation,if(a.strTaxIndicator='',' ',a.strTaxIndicator)strTaxIndicator "
	...
	public String funGetPOSName(String PosCode)
	{
	 ...
	 public Map<String, clsPromotionItems> funCalculatePromotions(String transType, String billFromKOTsList, String billNo, List<clsBillDtl> listOfSplitedGridItems,String tableNo,String areaCode,String voucherNo,HttpServletRequest request,List<clsWebPOSBillSettlementBean> listItemDtl ) throws Exception
	    {
	        ...
	        else if (transType.equalsIgnoreCase("AddKOTToBill"))//AddKOTToBill
	        {
	            ...
	            List<clsWebPOSBillSettlementBean> listKOTNos = new ArrayList<clsWebPOSBillSettlementBean>();
	            ...
	            {
	                boolean first = true;
	                for (clsWebPOSBillSettlementBean bean : listKOTNos)
	                {
	                    if (first)
	                    {
...
//	            String posDate = clsGlobalVarClass.getPOSDateForTransaction().split(" ")[0];
	        ...
	        else   // Direct Biller
	        {
	            ...
	            List<clsWebPOSBillSettlementBean> listDirectBillerItemDtl = listItemDtl;
	            String gDirectAreaCode=objPOSSetupUtility.funGetParameterValuePOSWise( clientCode,  posCode,  "gDirectAreaCode");
	            for (clsWebPOSBillSettlementBean objDirectBillerItems : listDirectBillerItemDtl)
	            {
	                sbPromo.setLength(0);
	                sbPromo.append("select c.strPromoCode,b.strBuyPromoItemCode,b.dblBuyItemQty,c.strDays"
	                        + ",c.tmeFromTime,c.tmeToTime,c.strPromotionOn,c.strGetPromoOn "
	  ...
	  public List funGetDocumentCode(String masterName) throws Exception
	    {
			...
			switch(masterName)
			{
				...
				case "POSDiscountMaster":
					sql = "select max(strDiscCode) from tbldischd";
					break;
