...
@Controller
public class clsPOSWaiterWiseItemReport {
	...
	private clsPOSGlobalFunctionsController objPOSGlobalFunctionsController; 
	 ...
	 @RequestMapping(value = "/frmPOSWaiterWiseItemReport", method = RequestMethod.GET)
		public ModelAndView funOpenForm(Map<String, Object> model,HttpServletRequest request)
		{
			...
			JSONArray jArryWaiterList =objPOSGlobalFunctionsController.funGetAllWaiterForMaster(strClientCode);
			for(int i =0 ;i<jArryWaiterList.size();i++)
			{
				JSONObject josnObjRet = (JSONObject) jArryWaiterList.get(i);
				waiterlist.add(josnObjRet.get("strWShortName"));
				hmWaiterData.put(josnObjRet.get("strWShortName").toString(),josnObjRet.get("strWaiterNo").toString());
			}
			...
			if("2".equalsIgnoreCase(urlHits)){
				return new ModelAndView("frmPOSWaiterWiseItemReport_1","command", new clsWebPOSReportBean());
			}else if("1".equalsIgnoreCase(urlHits)){
				return new ModelAndView("frmPOSWaiterWiseItemReport","command", new clsWebPOSReportBean());
			}else {
				return null;
			}
	 ...
	 @SuppressWarnings("rawtypes")
		...
		private void funReport(@ModelAttribute("command") clsWebPOSReportBean objBean, HttpServletResponse resp,HttpServletRequest req)
		{
			...
			try{
				...
				reportName = servletContext.getRealPath("/WEB-INF/reports/webpos/rptWaiterWiseItemReport.jrxml");
				imagePath = servletContext.getRealPath("/resources/images/company_Logo.png");//
				 ...
				 List<JasperPrint> jprintlist =new ArrayList<JasperPrint>();
					...
					String waiterCode = objBean.getStrWShortName();
					...
					jObjFillter.put("waiterCode",waiterCode);
					...
					JSONObject jObj = objGlobalFunctions.funPOSTMethodUrlJosnObjectData(clsPOSGlobalFunctionsController.POSWSURL+"/WebPOSReport/funWaiterWiseItemReport",jObjFillter);
					Gson gson = new Gson();
	 				Type listType = new TypeToken<List<clsBillDtl>>() {}.getType();
	 				JRBeanCollectionDataSource beanCollectionDataSource;
	 				
					listOfWaiterWiseItemSales = gson.fromJson(jObj.get("ListWaiterWiseItemDetail").toString(), listType);
		            ...
		            if (jprintlist.size()>0)
				    {
						...
						resp.setHeader("Content-Disposition", "inline;filename=WaiterWiseItemReport_"+ reportType +"_"+ strFromdate+"_To_"+strToDate+"_"+userCode+".pdf");
				    	...
				    	}else
				    	{
							...
							resp.setHeader("Content-Disposition", "inline;filename=WaiterWiseItemReport_"+ reportType +"_"+strFromdate+"_To_"+strToDate+"_"+userCode+".xls");
						    exporter.exportReport();
							servletOutputStream.flush();
							servletOutputStream.close();
				    	}
