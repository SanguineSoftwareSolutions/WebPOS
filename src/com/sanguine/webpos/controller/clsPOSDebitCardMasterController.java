...
//Save or Update DebitCardMaster
	...
	public ModelAndView funAddUpdate(@ModelAttribute("command") @Valid clsPOSDebitCardMasterBean objBean,BindingResult result,HttpServletRequest req)
	{
		...
		try
		{
			...
			String[] splitDateList = expiryDate.split("[-\\s]");
			String day = splitDateList[0];
			String month = splitDateList[1];
			String year = splitDateList[2];
...
//			objPOSDebitCardMaster.setStrSetExpiryDt((String) jObjSearchDetails.get(""));
			String expDate=jObjSearchDetails.get("dteExpiryDt").toString();
			String[] splitDateList = expDate.split("[-\\s]");
			String day = splitDateList[2];
			String month = splitDateList[1];
			String year = splitDateList[0];
			String dteExpiryDate = year +"-"+month+"-"+day;
			objPOSDebitCardMaster.setDteExpiryDt(dteExpiryDate);
